...
static void main_window_load(Window *window) {
  ...
  s_timeprompt_layer = text_layer_create(GRect(2, 2, 140, 50));
  text_layer_set_background_color(s_timeprompt_layer, GColorBlack);
  text_layer_set_text_color(s_timeprompt_layer, GColorGreen);
  text_layer_set_text(s_timeprompt_layer, "root ~$ time");
  text_layer_set_font(s_timeprompt_layer, fonts_get_system_font(FONT_KEY_GOTHIC_28));
  text_layer_set_text_alignment(s_timeprompt_layer, GTextAlignmentLeft);
  
  // Create time TextLayer
  s_time_layer = text_layer_create(GRect(2, 59, 140, 55));
